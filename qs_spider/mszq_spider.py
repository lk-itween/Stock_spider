...
h_txt = '''accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9
accept-encoding: gzip, deflate, br
accept-language: zh-CN,zh;q=0.9
referer: https://www.mszq.com/rzrq/target.jsp?date=2021.06.04&code=&type=2&curPage=2
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'https://www.mszq.com/rzrq/target.jsp'
    stock_name = '民生证券'
    post = False  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='741107', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def runningerror(self, p_data, encoding='utf-8', n=1):
        ...
        if n >= 5:
            return None
        time.sleep(np.random.random() * 1.1 + 1)
        data = list(self.post_data(page_num=p_data, N1=500))[0]
        res_text, html = get_html_ajex(self.url_ori, self.post, self.h_txt, data_nump,
                                       post_data_func=self.post_data)
        if res_text == '数据抓取失败。\n':
            self.label2show = '数据抓取失败。\n'
    ...
    def parse_html(self, html, exc=0):
        ...
        try:
            ...
            pagecount = re.search(r'共(\d+)条数据', html).group(1)
            pagecount = pagecount if type(pagecount) == int else int(float(pagecount))
            pageOrTotal = pagecount // self.page_size + 1
            if '<table' in html:
...
class rzrq(object):
    ...
    # Error running
    # 'Error' in html 时中运行
    def runningerror(self, p_data, referer=None, encoding='utf-8', n=1, label='融资标的'):
        self.label2show = f'第{n}次尝试\n'
        if n >= 5:
            return None
        time.sleep(np.random.random() * 1.1 + 1)
