...
h_txt = """Host: www.hxzq.cn
Connection: keep-alive
Cache-Control: max-age=0
Upgrade-Insecure-Requests: 1
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9
Referer: http://www.hxzq.cn/index.php?c=rzrqsy&f=db&pageid=2
...
class db(object):
    ...
    urlo = 'http://www.hxzq.cn/index.php?c=rzrqsy&f=db'
    stock_name = '宏信证券'
    post = False
    postbut = False
    scrollwithauto = True
    ...
    # 运行函数
    def running_main(self, p_data, referer=None, encoding='utf-8', n=1, label='可担保物'):
        ...
        try:
            ...
            time.sleep(25)
            return self.running_main(p_data, referer=referer, encoding=encoding, n=n + 1, label=label)
    ...
    def parse_html(self, html, label='可担保物', exc=0):
        ...
        df_columns = {'市场': '市场', 0: '证券代码', 1: '证券简称', 2: '折算率',
                      'MODIFY_DATE': '日期'}
...
class rzrq(object):
    ...
    date0 = datetime.datetime.now()
    date1 = date0.strftime('%Y%m%d')
    urlo = 'http://www.hxzq.cn/index.php?c=rzrqsy&f=bd'
    stock_name = '宏信证券'
    post = False
    postbut = False
    scrollwithauto = True
    ...
    def running_main(self, p_data, referer=None, encoding='utf-8', n=1, label='融资标的'):
        ...
        try:
            ...
            n += 1
            self.label2show = f'第{n}次尝试\n'
            return self.running_main(p_data, referer=referer, encoding=encoding, n=n, label=label)
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        html1 = re.search(r'[\s\S]+<tr>', html).group(0)
        df = pd.read_html('<table>' + html1 + '</table>')[0].dropna()
        df_columns = {'3': '市场', 0: '证券代码', 1: '证券简称', 2: '融资保证金比例',
                      'MODIFY_DATE': '日期'}
        df.rename(columns=df_columns, inplace=True)
        df = trans_data(df, self.date)
        return df, page_total
