...
h_txt = '''Host: www.cczq.com
Connection: keep-alive
Content-Length: 44
Content-Type: application/x-www-form-urlencoded
Accept: */*
Origin: http://www.cczq.com
Referer: http://www.cczq.com/home/buscenter/sfc/info/margin.html
...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'http://www.cczq.com/webapi/api/secfinMarginList'
    stock_name = '川财证券'
    post = True  # 数据请求方式
    ...
    def parse_html(self, html, exc=0):
        ...
        try:
            df_json = json.loads(html)['page']
            pageOrTotal = df_json['totalCount']
        ...
        df_columns = {'sssc': '市场', 'secCode': '证券代码', 'secName': '证券简称', 'marginRatio100Str': '折算率',
                      'createdTime': '日期'}
        #######################################
...
class rzrq(object):
    ...
    date1 = date0.strftime('%Y%m%d')
    ########  需要修改的部分
    url_ori = 'http://www.cczq.com/webapi/api/secfinUnderList'
    stock_name = '川财证券'
    post = True  # 数据请求方式
    scrollwithauto = False  # 是否翻页抓取数据
    combineflag = False  # 融资融券是否在同一页面上
    page_size = 10  # 页面数据量大小
    ...
    def thread_main(self):
        df_list = []
        rzrq = zip(['rz.js', 'rq.js'], ['融资标的', '融券标的'])
        rzrq_combine = zip([''], ['融资融券'])
        rzrq = rzrq_combine if self.combineflag else rzrq
        for _, label in rzrq:
            ########  需要修改的部分
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='融资', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                html1 = re.search(r'[\s\S]+<tr>', html).group(0)
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
        df_columns = {'sssc': '市场', 'secCode': '证券代码', 'secName': '证券简称', 'rz': '融资保证金比例',
                      'rq': '融券保证金比例', 'createdTime': '日期', 'marginRatio100Str': '保证金比例', 'undersecType': '标的类别'}
        df.rename(columns=df_columns, inplace=True)
        if '保证金比例' in df.columns:
            df = df[df['标的类别'] == label[:2]].rename(columns={'保证金比例': label[:2] + '保证金比例'})
        #######################################
        df = trans_data(df, self.date)
        return df, pageOrTotal
