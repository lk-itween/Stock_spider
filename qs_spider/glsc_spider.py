...
h_txt = '''Host: www.glsc.com.cn
Connection: keep-alive
Upgrade-Insecure-Requests: 1
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9
Referer: http://www.glsc.com.cn/glzq/glzqrzrq/kcdbzjzq.jsp
...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'http://www.glsc.com.cn/glzq/glzqrzrq/kcdbzjzqList.html'
    stock_name = '国联证券'
    post = False  # 数据请求方式
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'bourse': '市场', '证券代码': '证券代码', '证券名称': '证券简称', '公司折算率(%)': '折算率',
...
class rzrq(object):
    ...
    def post_data(self, page_num=1, label='融资融券', N1=page_size):
        ...
        每次重新编辑需要修改
        """
        rzrq_dict = {'融资标的': 'http://www.glsc.com.cn/glzq/glzqrzrq/rzbdzqList.html',
                     '融券标的': 'http://www.glsc.com.cn/glzq/glzqrzrq/bdzqc.jsp',
                     '融资融券': '101202', }
        datapost = {
        }
        dataget = '?' + '&'.join([f'{x}={y}' for x, y in datapost.items()])
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            pageOrTotal = 0
            if label == '融券标的':
                html = re.search(r'融券标的[\s\S]*?<table[\s\S]*?/table>', html).group(0)
            if '<table' in html:
                html = re.search(r'<table[\s\S]*?/table>', html).group(0)
                html = re.sub(r'\s+', ' ', html)
                df = pd.read_html(html, header=0)[0].dropna()
                ...
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
        #             pageOrTotal = pageOrTotal if pageOrTotal else 2000
        df_columns = {'bourse': '市场', '证券代码': '证券代码', '证券名称': '证券简称', 'fiMarginRatio': '融资保证金比例',
                      'slMarginRatio': '融券保证金比例', 'fbrq': '日期', '保证金比例(%)': '保证金比例', '保证金比例（%）': '保证金比例',
                      'undersecType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
