...
h_txt = '''Host: www.xsdzq.cn
Connection: keep-alive
Content-Length: 26
Accept: text/plain, */*; q=0.01
X-Requested-With: XMLHttpRequest
Content-Type: application/x-www-form-urlencoded; charset=UTF-8
Origin: http://www.xsdzq.cn
Referer: http://www.xsdzq.cn/rzrqxls?sflg=1
...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'http://www.xsdzq.cn/getPageRzrqList_ZQMD'
    stock_name = '新时代证券'
    post = True  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label=1, N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'jys': '市场', '证券代码': '证券代码', '证券简称': '证券简称', '折算率': '折算率',
                      'reportdate': '日期'}
        #######################################
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
            pageOrTotal = pageOrTotal if pageOrTotal else 2000
        df_columns = {'jys': '市场', '证券代码': '证券代码', '证券简称': '证券简称', 'rz': '融资保证金比例',
                      'rq': '融券保证金比例', 'reportdate': '日期', '保证金比例': '保证金比例', 'undersecType': '标的类别'}
