...
Origin: https://www.stocke.com.cn
Sec-Fetch-Site: same-origin
Sec-Fetch-Mode: cors
Sec-Fetch-Dest: empty
Referer: https://www.stocke.com.cn/main/gsyw/ywzc/rzrq/gsxx/index.shtml
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'https://www.stocke.com.cn/servlet/json'
    stock_name = '浙商证券'
    post = True  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='501051', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'exchange': '市场', 'fund_code': '证券代码', 'fund_acco': '证券简称', 'conversion': '折算率',
                      'display_date': '日期', '状态': '状态', }
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                html1 = re.sub(r'\s+', ' ', html1)
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
        df_columns = {'exchange': '市场', 'fund_code': '证券代码', 'fund_acco': '证券简称', 'financing_ratio': '融资保证金比例',
                      'margin_ratio': '融券保证金比例', 'display_date': '日期', 'converRate': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
