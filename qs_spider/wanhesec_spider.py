...
h_txt = '''Host: www.wanhesec.com.cn
Connection: keep-alive
Content-Length: 67
Accept: application/json, text/javascript, */*; q=0.01
X-Requested-With: XMLHttpRequest
Content-Type: application/x-www-form-urlencoded; charset=UTF-8
Origin: http://www.wanhesec.com.cn
Referer: http://www.wanhesec.com.cn/main/views/creditTransaction/rzrq/bzj/index.html
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'http://www.wanhesec.com.cn/servlet/json'
    stock_name = '万和证券'
    post = True  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='100056', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'marketname': '市场', 'stkcode': '证券代码', 'stkname': '证券简称', 'pledgerate': '折算率',
                      'updatetime': '日期', '状态': '状态', }
...
class rzrq(object):
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            df_use_col = df.columns[[1, 2, 3]]
            df.dropna(subset=df_use_col, inplace=True)
        df_columns = {'marketname': '市场', 'stkcode': '证券代码', 'stkname': '证券简称', '融资保证金比例': '融资保证金比例',
                      '融券保证金比例': '融券保证金比例', 'updatetime': '日期', '保证金比例': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        df['融资保证金比例'] = '是'
        df['融券保证金比例'] = '是'
        #######################################
