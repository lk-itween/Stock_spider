...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'https://www.ccnew.com/cgi-bin/marginTrading/CoefficientAction'
    stock_name = '中原证券'
    post = False  # 数据请求方式
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'市场': '市场', '证券代码': '证券代码', '证券简称': '证券简称', '折算率': '折算率',
...
class rzrq(object):
    ...
    date1 = date0.strftime('%Y%m%d')
    ########  需要修改的部分
    url_ori = 'http://www.ccnew.com/cgi-bin/marginTrading/StockAction'
    stock_name = '中原证券'
    post = False  # 数据请求方式
    scrollwithauto = False  # 是否翻页抓取数据
    page_size = 10  # 页面数据量大小
