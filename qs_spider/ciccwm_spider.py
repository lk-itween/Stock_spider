...
origin: https://www.ciccwm.com
referer: https://www.ciccwm.com/fund/guarantee.jsp
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'https://www.ciccwm.com/web2016/guaranteeServlet'
    stock_name = '中国中投'
    post = True  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='904005', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def thread_main(self):
        df_list = []
        ########  需要修改的部分
        p_data = None  # list(self.post_data(page_num=2))[0]
        url_load = 'https://www.ciccwm.com/web2016/guaranteeServlet'
        res_text, html = get_html_load(url_load, self.h_txt, post_data_func=self.post_data)
        if res_text == '数据抓取失败。\n':
            self.label2show = '数据抓取失败。\n'
            return None, None
        encoding = chardet.detect(html)['encoding']
        encoding = 'gbk' if encoding == 'GB2312' and chardet.detect(html)['confidence'] <= 0.99 else encoding
        html_load = html.decode(encoding)
        pageOrTotal = int(re.search(r'count":(\d+)', html_load).group(1))
        encoding = 'utf-8'
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'exchange': '市场', 'stockId': '证券代码', 'stockName': '证券简称', 'rate': '折算率',
                      'createTime': '日期', '状态': '状态', }
...
class rzrq(object):
    ...
    date1 = date0.strftime('%Y%m%d')
    ########  需要修改的部分
    urlo = 'https://www.ciccwm.com/web2016/targetStockServlet'
    stock_name = '中国中投'
    post = True  # 数据请求方式
    postbut = False  # 数据请求方式 为 post但是要按get方式构建url
    scrollwithauto = True  # 是否翻页抓取数据
    ...
    def thread_main(self):
        ...
        #######################################
        for label in rzrq:
            res_text, html = get_html_load(rzrq_url[label], self.h_txt, post_data_func=self.post_data)
            if res_text == '数据抓取失败。\n':
                self.label2show = '数据抓取失败。\n'
                return None, None
            encoding = chardet.detect(html)['encoding']
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        df_columns = {'exchange': '市场', 'stockId': '证券代码', 'stockName': '证券简称', 'guaranteeMoney': '融资保证金比例',
                      'guaranteeStock': '融券保证金比例', 'createTime': '日期', 'ratio': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
