...
Referer: https://www.cindasc.com/osoa/views/xdyw/xyjy/rzrq/zqcx/index.html
...
class db(object):
    ...
    url_ori = 'https://www.cindasc.com/servlet/json'
    stock_name = '信达证券'
    post = True
    page_size = 10
...
class rzrq(object):
    ...
    # 多线程，设置线程池
    def thread_main(self):
        for funcNo, label in zip(['2000100', '2000101'], ['融资标的', '融券标的']):
    ...
    # 构造请求数据字典
    def post_data(self, funcNo='2000100', page_num=1, N1=10):
        data = data = {
            'funcNo': funcNo,
            'currentPage': page_num,
            'numPerPage': N1,
    ...
    def parse_html(self, html, label='融资标的'):
        ...
        page_total = page_total if type(page_total) == int else int(float(page_total))
        df = pd.DataFrame(df_json['DataSet0'])
        df_columns = {'secu_code': '证券代码', 'secu_name': '证券简称', 'rzbzjbl': '融资保证金比例',
                      'rqbzjbl': '融券保证金比例', 'creat_date': '日期'}
        df.rename(columns=df_columns, inplace=True)
        df = trans_data(df, self.date)
        return df, page_total
