...
h_txt = """Host: www.ykzq.com
Connection: keep-alive
Accept: application/json, text/javascript, */*; q=0.01
X-Requested-With: XMLHttpRequest
Referer: http://www.ykzq.com/channel/93.html
...
class db(object):
    ...
    urlo = 'http://www.ykzq.com/api/financing/offset/page?'
    ...
    def parse_html(self, html, label='可担保物', exc=0):
        df_json = json.loads(html)['get_response']['offsets']
        page_total = df_json['totalItemsCount']
        page_total = page_total if type(page_total) == int else int(float(page_total))
        df = pd.DataFrame(df_json['items'])
        df_columns = {'SecMarket': '市场', 'stockCode': '证券代码', 'stockName': '证券简称', 'discountRate': '折算率',
                      'EffectiveDate': '日期'}
...
class rzrq(object):
    ...
    date0 = datetime.datetime.now()
    date1 = date0.strftime('%Y%m%d')
    urlo = 'http://www.ykzq.com/api/financing/signs/page?'
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        df_json = json.loads(html)['get_response']['signs']
        page_total = df_json['totalItemsCount']
        page_total = page_total if type(page_total) == int else int(float(page_total))
        df = pd.DataFrame(df_json['items'])
        df_columns = {'SecMarket': '市场', 'stockCode': '证券代码', 'stockName': '证券简称', 'financingRate': '融资保证金比例',
                      'securityRate': '融券保证金比例', 'EffectiveDate': '日期'}
        df.rename(columns=df_columns, inplace=True)
        df = trans_data(df, self.date)
        return df, page_total
