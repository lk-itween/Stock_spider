...
Referer: https://www.bocichina.com/ListDetail/List-RZRQ/?tab=4
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'https://www.bocichina.com/file/data/datarzrq3.json'
    stock_name = '中银国际'
    post = False  # 数据请求方式
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'market': '市场', 'stockCode': '证券代码', 'stockName': '证券简称', 'ratio': '折算率',
                      'modify_date': '日期', '状态': '状态', }
...
class rzrq(object):
    ...
    date1 = date0.strftime('%Y%m%d')
    ########  需要修改的部分
    urlo = 'https://www.dtsbc.com.cn/servlet/json'
    stock_name = '中银国际'
    post = False  # 数据请求方式
    postbut = False  # 数据请求方式 为 post但是要按get方式构建url
    scrollwithauto = False  # 是否翻页抓取数据
    ...
    def thread_main(self):
        ...
        rzrq_url = {'融资标的': 'https://www.bocichina.com/file/data/datarzrq1.json',
                    '融券标的': 'https://www.bocichina.com/file/data/datarzrq2.json',
                    '融资融券': '904006', }

        #######################################
        for label in rzrq:
            #             p_data = list(self.post_data(label=label))[0]
            #             html = self.get_html_ajex(self.url_ori, p_data)
            res_text, html = get_html_ajex(rzrq_url[label], self.post, self.h_txt,
                                           post_data_func=self.post_data)
            if res_text == '数据抓取失败。\n':
                self.label2show = '数据抓取失败。\n'
            encoding = chardet.detect(html)['encoding']
            ...
            if pageOrTotal:
                if pageOrTotal == 2000:
                    pageOrTotal = binary_html_get_page(self.url_ori, self.h_txt, encoding, pageOrTotal,
                                                       self.post_data, self.parse_html, label)
                    if not pageOrTotal:
                        self.label2show = '数据抓取失败。\n'
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        df_columns = {'market': '市场', 'stockCode': '证券代码', 'stockName': '证券简称', 'rzbzjbl': '融资保证金比例',
                      '融券保证金比例': '融券保证金比例', 'xgrq': '日期', 'ratio': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        df.rename(columns=df_columns, inplace=True)
