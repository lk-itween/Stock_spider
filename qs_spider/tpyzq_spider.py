...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'https://www.tpyzq.com/servlet/RzbdAction?type=3'
    stock_name = '太平洋证券'
    post = True  # 数据请求方式
...
class rzrq(object):
    ...
    date1 = date0.strftime('%Y%m%d')
    ########  需要修改的部分
    urlo = 'https://www.tpyzq.com/servlet/RzbdAction?type=2'
    stock_name = '太平洋证券'
    post = True  # 数据请求方式
    postbut = True  # 数据请求方式 为 post但是要按get方式构建url
    scrollwithauto = False  # 是否翻页抓取数据
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                html1 = re.search(r'[\s\S]+<tr>', html).group(0)
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
        df_columns = {'市场': '市场', '证劵代码': '证券代码', '证劵简称': '证券简称', 'financingtarget': '融资保证金比例',
                      'margintradingtarget': '融券保证金比例', 'createtime': '日期', '保证金比例': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
