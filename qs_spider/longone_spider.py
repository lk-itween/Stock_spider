...
h_txt = '''Host: www.longone.com.cn
Connection: keep-alive
Content-Length: 25
Accept: */*
X-Requested-With: XMLHttpRequest
Content-Type: application/x-www-form-urlencoded; charset=UTF-8
Origin: http://www.longone.com.cn
Referer: http://www.longone.com.cn/main/rzrq/rzrqyw/tbfund/index.html
...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'http://www.longone.com.cn/servlet/stock/RzrqAction'
    stock_name = '东海证券'
    post = True  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='getDbzqData', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
            pageOrTotal = pageOrTotal if pageOrTotal else 2000
        df_columns = {'jys': '市场', '证券代码': '证券代码', '证券简称': '证券简称', '融资保证金比例': '融资保证金比例',
                      '融券保证金比例': '融券保证金比例', '发布日期': '日期', '保证金比例': '保证金比例', 'undersecType': '标的类别'}
