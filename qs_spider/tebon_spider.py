...
Origin: https://www.tebon.com.cn
Sec-Fetch-Site: same-origin
Sec-Fetch-Mode: cors
Sec-Fetch-Dest: empty
Referer: https://www.tebon.com.cn/main/businessGuide/economicBusiness/marginTrading/xgsj/kcdbzjzj/index.html
...
class db(object):
    ...
    urlo = 'https://www.tebon.com.cn/servlet/json'  # {date}
    stock_name = '德邦证券'
    post = True
    postbut = False
    ...
    def thread_main(self):
        ...
        if data.empty:
            ...
            return data
        self.df_data = data
        self.df_data = modify_field(self.df_data)
    ...
    def parse_html(self, html, label='可担保物', exc=0):
        ...
        df_columns = {'stock_code': '证券代码', 'stock_short_name': '证券简称', 'discount_rate': '折算率',
                      'create_date': '日期'}
...
class rzrq(object):
    ...
    date0 = datetime.datetime.now()
    date1 = date0.strftime('%Y%m%d')
    urlo = 'https://www.tebon.com.cn/servlet/json'
    stock_name = '德邦证券'
    post = True
    postbut = False
    scrollwithauto = False
    ...
    def thread_main(self):
        ...
        for label in ['融资标的', '融券标的']:
            p_data = list(self.post_data(N1=1, label=label))[0]
            data = get_data(self.url_ori, self.post, self.h_txt, p_data, self.page_start, self.page_end,
                            self.scrollwithauto, label=label,
                            running_func=self.running_main, post_data_func=self.post_data,
                            parse_html_func=self.parse_html)
    ...
    def running_main(self, p_data, referer=None, encoding='utf-8', n=1, label='融资标的'):
        ...
        try:
            ...
            time.sleep(25)
            return self.running_main(p_data, label=label)
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        df = pd.DataFrame(df_json['data'])
        if label == '融资标的':
            df_columns = {'bond_code': '证券代码', 'bond_name': '证券简称', 'financing_value': label[:2] + '保证金比例',
                          'create_date': '日期'}
        else:
            df_columns = {'bond_code': '证券代码', 'bond_name': '证券简称', 'margin_value': label[:2] + '保证金比例',
                          }
