...
Origin: https://www.cnht.com.cn
Sec-Fetch-Site: same-origin
Sec-Fetch-Mode: cors
Sec-Fetch-Dest: empty
Referer: https://www.cnht.com.cn/main/Business_introduction/Margin_trading/infopublicity/bzjRate/index.shtml
...
class db(object):
    ...
    url_ori = 'https://www.cnht.com.cn/servlet/json'
    stock_name = '恒泰证券'
    ...
    def parse_html(self, html):
        ...
        df_columns = {'市场': '市场', 'product_code': '证券代码', 'product_name': '证券简称', 'margin_rate': '折算率',
                      'create_date': '日期'}
...
class rzrq(object):
    ...
    def parse_html(self, html, label='融资标的'):
        ...
        page_total = page_total if type(page_total) == int else int(float(page_total))
        df = pd.DataFrame(df_json['data'])
        df_columns = {3: '市场', 'product_code': '证券代码', 'product_name': '证券简称', 'financing_margin_rate': '融资保证金比例',
                      'create_date': '日期'}
        df.rename(columns=df_columns, inplace=True)
        df = trans_data(df, self.date)
        return df, page_total
