...
Origin: https://stock.pingan.com
Sec-Fetch-Site: same-origin
Sec-Fetch-Mode: cors
Sec-Fetch-Dest: empty
Referer: https://stock.pingan.com/static/webinfo/margin/business.html?businessType=1
...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'https://stock.pingan.com/restapi/contentservice/option/openWeb/queryRzrqStockList'
    stock_name = '平安证券'
    post = True  # 数据请求方式
    scrollwithauto = False  # 是否翻页抓取数据
    page_size = 18  # 页面数据量大小
    ...
    def post_data(self, page_num=1, label='101202', N1=page_size):
        ...
        """
        datapost = {
            'body': {'pageNo': page_num, 'pageSize': str(N1), 'type': 'kcdzq',
                     'settDate': self.date, 'secuCode': '', 'market': '00'},
            'channel': 'pa18',
            'requestId': '86573147527b184a182453933c81f15b',
        }
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'bourse': '市场', 'secuCode': '证券代码', 'secuName': '证券简称', 'collDiscRatio': '折算率',
                      'insert_date': '日期'}
        #######################################
...
class rzrq(object):
    ...
    def post_data(self, page_num=1, label='融资融券', N1=page_size):
        ...
        rzrq_dict = {'融资标的': 0,
                     ...
                     '融资融券': '101202', }
        datapost = {
            'body': {'pageNo': page_num, 'pageSize': str(N1), 'type': 'bdzq',
                     'settDate': self.date, 'secuCode': '', 'market': '00'},
            'channel': 'pa18',
            'requestId': '5196c17f9077a69760e84bdcbac91c62',
        }
        dataget = '?' + '&'.join([f'{x}={y}' for x, y in datapost.items()])
        yield json.dumps(datapost) if self.post else dataget
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
            pageOrTotal = pageOrTotal if pageOrTotal else 2000
        df_columns = {'bourse': '市场', 'secuCode': '证券代码', 'secuName': '证券简称', 'fiMarginRatio': '融资保证金比例',
                      'slMarginRatio': '融券保证金比例', 'fbrq': '日期', '保证金比例': '保证金比例',
                      'undersecType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
