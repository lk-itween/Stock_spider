...
Origin: https://www.wkzq.com.cn
Sec-Fetch-Site: same-origin
Sec-Fetch-Mode: cors
Sec-Fetch-Dest: empty
Referer: https://www.wkzq.com.cn/wkzq/views/business/honor/dbwframe.html
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'https://www.wkzq.com.cn/servlet/json'
    stock_name = '五矿证券'
    post = True  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='905501', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'jys': '市场', 'zqdm': '证券代码', 'zqmc': '证券简称', 'zsl': '折算率',
                      'fbrq': '日期'}
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
            pageOrTotal = pageOrTotal if pageOrTotal else 2000
        df_columns = {'jys': '市场', 'zqdm': '证券代码', 'zqmc': '证券简称', 'rzbd': '融资保证金比例',
