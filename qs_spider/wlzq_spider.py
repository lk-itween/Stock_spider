...
h_txt = '''Host: www.wlzq.cn
Connection: keep-alive
Accept: */*
X-Requested-With: XMLHttpRequest
Content-Type: application/x-www-form-urlencoded
Referer: http://www.wlzq.cn/main/xyyw/rzrq/zqcx/rzrqdbzq/index.shtml
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'http://www.wlzq.cn/servlet/TZLLAction'
    stock_name = '万联证券'
    post = False  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='KCDBZJ', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
...
class rzrq(object):
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        df_columns = {'交易所': '市场', '证券代码': '证券代码', '证券简称': '证券简称', '融资保证金比例': '融资保证金比例',
                      '融券保证金比例': '融券保证金比例', 'STATE_TIME': '日期', '保证金比例': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
