...
h_txt = '''Host: www.essence.com.cn
Connection: keep-alive
Accept: application/json, text/javascript, */*; q=0.01
X-Requested-With: XMLHttpRequest
Referer: http://www.essence.com.cn/business/security/rzrq/deposit
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'http://www.essence.com.cn/api/security'
    stock_name = '安信证券'
    post = False  # 数据请求方式
    ...
    def parse_html(self, html, exc=0):
        ...
        try:
            ...
            try:
                pageOrTotal = df_json['total'] // self.page_size + 1
        ...
        df_columns = {'market': '市场', 'secuCode': '证券代码', 'secuName': '证券简称', 'collatRatio': '折算率',
...
class rzrq(object):
    ...
    date1 = date0.strftime('%Y%m%d')
    ########  需要修改的部分
    urlo = 'http://www.essence.com.cn/api/security?fislFlag=1'
    stock_name = '安信证券'
    post = False  # 数据请求方式
    postbut = False  # 数据请求方式 为 post但是要按get方式构建url
    scrollwithauto = True  # 是否翻页抓取数据
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        df_columns = {'market': '市场', 'secuCode': '证券代码', 'secuName': '证券简称', 'fiMarginRatio': '融资保证金比例',
                      'slMarginRatio': '融券保证金比例', 'effectiveDate': '日期', 'BAIL_RATIOO': '保证金比例', '保证金比例（%）': '保证金比例',
        ...
        #         df = df[df['状态'] == '0']
        #######################################
        df['证券代码'] = df['证券代码'].map(lambda x: str(int(x)))
