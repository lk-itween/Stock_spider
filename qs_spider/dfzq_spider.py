...
Referer: https://www.dfzq.com.cn/osoa/views/main/mainbusiness/securitiesfinance/margintrading/conversionrates/index.shtml
...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'https://www.dfzq.com.cn/servlet/json'
    stock_name = '东方证券'
    post = True  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='501003', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'exchange': '市场', 'securitiescode': '证券代码', 'securitieshort': '证券简称', 'convertrate': '折算率',
                      'modifytime': '日期', '状态': '状态', }
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
        #             pageOrTotal = pageOrTotal if pageOrTotal else 2000
        df_columns = {'exchange': '市场', 'securitiescode': '证券代码', 'securitieshort': '证券简称',
                      'financingtarget': '融资保证金比例',
                      'margintradingtarget': '融券保证金比例', 'createtime': '日期', 'converRate': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
