...
Origin: https://www.lczq.com
Sec-Fetch-Site: same-origin
Sec-Fetch-Mode: cors
Sec-Fetch-Dest: empty
Referer: https://www.lczq.com/main/xyyw/dbzq/index.html
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'https://www.lczq.com/servlet/json'
    stock_name = '联储证券'
    post = True  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='905003', N1=page_size):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'market': '市场', 'stock_code': '证券代码', 'name': '证券简称', 'conversion_ate': '折算率',
                      'updatetime': '日期', '状态': '状态', }
...
class rzrq(object):
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        df_columns = {'market': '市场', 'stock_code': '证券代码', 'name': '证券简称', 'financing_ratio': '融资保证金比例',
                      'securities_ratio': '融券保证金比例', 'updatetime': '日期', '保证金比例': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
