...
Referer: https://www.foundersc.com/creditMarginPublicNewsAssure/index.jhtml
...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'https://www.foundersc.com/infoMargin/getFzRzrqSecuritiesList.jspx'
    stock_name = '方正证券'
    post = True  # 数据请求方式
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'marketName': '市场', 'stockCode': '证券代码', 'stockName': '证券简称', 'rate': '折算率',
                      'tradeDate': '日期', '状态': '状态', }
...
class rzrq(object):
    ...
    date1 = date0.strftime('%Y%m%d')
    ########  需要修改的部分
    url_ori = 'https://www.foundersc.com/infoMargin/getFzRzrqRatioAndSecuritiesNewList.jspx'
    stock_name = '方正证券'
    post = True  # 数据请求方式
    scrollwithauto = False  # 是否翻页抓取数据
    page_size = 10  # 页面数据量大小
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
        #             pageOrTotal = pageOrTotal if pageOrTotal else 2000
        df_columns = {'marketName': '市场', 'stockCode': '证券代码', 'stockName': '证券简称', 'finRate': '融资保证金比例',
                      'saleRate': '融券保证金比例', 'tradeDate': '日期', 'converRate': '保证金比例', '保证金比例（%）': '保证金比例',
                      'underlyingType': '标的类别'}
        #         df['融资保证金比例'] = '是'
        #         df['融券保证金比例'] = '是'
        #######################################
