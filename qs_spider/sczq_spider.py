...
Referer: https://www.sczq.com.cn/credit.html
...
class db(object):
    ...
    ########  需要修改的部分
    url_ori = 'https://www.sczq.com.cn/Article/Index/creditinfo.html'
    stock_name = '首创证券'
    post = False  # 数据请求方式
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'jys': '市场', 'code': '证券代码', 'name': '证券简称', 'rate': '折算率',
                      'reportdate': '日期'}
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        try:
            ...
            if '<table' in html:
                ...
                html1 = re.search(r'[\s\S]+<tr>', html).group(0)
                df = pd.read_html('<table>' + html1 + '</table>', header=0)[0].dropna()
        df_columns = {'jys': '市场', 'code': '证券代码', 'name': '证券简称', 'rz': '融资保证金比例',
                      'rq': '融券保证金比例', 'reportdate': '日期', 'rate': '保证金比例', 'undersecType': '标的类别'}
        df.rename(columns=df_columns, inplace=True)
