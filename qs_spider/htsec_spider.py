...
Referer: https://www.htsec.com/ChannelHome/2019090402/index.shtml
...
class db(object):
    ...
    ########  需要修改的部分
    urlo = 'https://www.htsec.com/htAPI/ht/rzrq/bdzqList'
    stock_name = '海通证券'
    post = False  # 数据请求方式
    ...
    # 构造请求数据字典
    def post_data(self, page_num=1, label='2', N1=page_size, jys='1'):
        """ post: data参数
        每次重新编辑需要修改
        """
        datapost = {
        ...
        if pageOrTotal:
            if pageOrTotal == 2000:
                pageOrTotal = binary_html_get_page(self.url_ori, self.h_txt, encoding, pageOrTotal,
                                                   self.post_data, self.post_data, label='label')
                if not pageOrTotal:
                    self.label2show = '数据抓取失败。\n'
    ...
    def parse_html(self, html, exc=0):
        ...
        df_columns = {'marketCode': '市场', 'productCode': '证券代码', 'productName': '证券简称', 'stockConvertRate': '折算率',
                      'etlDate': '日期', 'assure_status': '状态', }
...
class rzrq(object):
    ...
    def parse_html(self, html, label='融资标的', exc=0):
        ...
        df_columns = {'marketCode': '市场', 'productCode': '证券代码', 'productName': '证券简称', 'ifFinancing': '融资保证金比例',
                      'ifBorrowStock': '融券保证金比例', 'etlDate': '日期', 'BAIL_RATIOO': '保证金比例', '保证金比例（%）': '保证金比例',
